...
-- 53. WIDGETS — multi-widget upgrade. Section 13 originally put an inline
-- `unique` on widgets.business_id (one widget per business); this upgrades
-- it to "many widgets per business", each one optionally bound to a specific
-- AI agent, matching the dashboard Widget page (list + create/edit/delete +
-- embeddable script tag per widget).
do $$
declare
  cname text;
begin
...
alter table widgets add column if not exists name text not null default 'Main Widget';
alter table widgets add column if not exists agent_id uuid references ai_agents(id) on delete set null;
alter table widgets add column if not exists position text not null default 'bottom-right'
  check (position in ('bottom-right', 'bottom-left'));
alter table widgets add column if not exists theme text not null default 'light'
  check (theme in ('light', 'dark'));
alter table widgets add column if not exists impressions integer not null default 0;
alter table widgets add column if not exists interactions integer not null default 0;
